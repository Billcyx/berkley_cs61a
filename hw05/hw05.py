...
def make_withdraw(balance, password):
    ...
    def withdraw(balance, password):
        nonlocal correct_password, mistake_list, total_balance

        if password != correct_password:

            if len(mistake_list) == 3:
                return "Too many incorrect attempts. Attempts: " + str(mistake_list)
            else:
                mistake_list.append(password)
...
def repeated(t, k):
    ...
    while not index == 1:
            ...
            following_ele = next(t)

            if following_ele == pre_ele:
                target_element =pre_ele
                index = index - 1 

            else:
                index = k

            pre_ele = following_ele
...
def merge(incr_a, incr_b):
    ...
    while(not next_a == None or not next_b == None):
        ...
        if next_a == None and not next_b == None:
            if next_b != pre_ele:
                yield next_b
            pre_ele = next_b
            next_b = next(iter_b, None)

        if not next_a == None and next_b == None:
            if next_a != pre_ele:
                yield next_a
            pre_ele = next_a
            next_a = next(iter_a, None)

        if not next_a == None and not next_b == None:
            if next_a > next_b:
                if next_b != pre_ele:
                    yield next_b
                pre_ele = next_b
