...
def missing_digits(n):
    ...
    if n // 10 == 0:
        ...
        if digit > n // 10 % 10:
            miss_num = digit - n // 10 % 10 - 1
        return miss_num + missing_digits(n//10)
...
def count_change(total):
    ...
    def find_largest_coin(total):
        coin_value = 1
        while coin_value*2 <= total:
            coin_value = coin_value*2
        return coin_value

    def partition_total(total, largest_coin_value):
        if total == 1 or total == 0:
            return 1
        if total < 0:
            return 0
        if largest_coin_value < 1:
            return 0
        return partition_total(total-largest_coin_value, largest_coin_value) + partition_total(total, largest_coin_value/2)

    return partition_total(total, find_largest_coin(total))
