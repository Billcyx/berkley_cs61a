...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            assert place.ant.can_contain(self) == True or self.can_contain(place.ant) == True, 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place is not None and place.is_hive == False and current_range <= self.upper_bound:

            if len(place.bees) != 0 and current_range >= self.lower_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        self.place.bees = list(list_bee)    

        if self.health <= amount:    

            for bee in list(list_bee):
        ...
        while place_behind is not None:
            if place_behind.ant:
                ...
                if ant.is_container:
                    ...
                    if ant.ant_contained is not None and ant.ant_contained.double_damage == False:
                        ant.ant_contained.double()
                        ant.ant_contained.double_damage = True
                elif ant.double_damage == False:
                    ant.double()
                    ant.double_damage = True
